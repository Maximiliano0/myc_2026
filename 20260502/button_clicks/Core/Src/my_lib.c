...
volatile uint8_t debounce_ctr[BTN_COUNT] = {0}; // DeBounce Counter per Button

/* Button Table -------------------------*/
static Button_t buttons[BTN_COUNT] = {
	[BTN_USER] = { SWITCH_PORT, SWITCH_PIN, 0 },
	// [BTN_NEXT] = { GPIOB, GPIO_PIN_4, 0 },
};
...
/* Filtered Read Pin */
Click_State  CLICK_Detect(Button_Id id){
	...
	switch(b->state){
		case 0: // Non Pressed
			button_state = (HAL_GPIO_ReadPin(b->port, b->pin)==SWITCH_ON)?	Pressed:Non_Pressed;
			if(button_state == Pressed){
				debounce_ctr[id] = DeBounce_Delay;
				b->state = 1;
			}
			break;
		case 1: // Pressed - esperando que termine el filtro anti-rebote
			if(debounce_ctr[id] == 0){ // Termino el Delay
				button_state = (HAL_GPIO_ReadPin(b->port, b->pin)==SWITCH_ON)?	Pressed:Non_Pressed;
				if(button_state == Pressed)	b->state = 2; // Confirmado: esperar release
				else b->state = 0; // Era rebote/glitch: descartar
			}
			break;
...
void TIM3_IRQHandler(void)
{
  ...
  for(uint8_t i = 0; i < BTN_COUNT; i++){
	  if(debounce_ctr[i] > 0) debounce_ctr[i]--;
  }
