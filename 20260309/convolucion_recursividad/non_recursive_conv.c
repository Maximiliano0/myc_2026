...
int main(void)
{
    ...
    for (i = 0; i < MUESTRAS_ENTRADA; i++) {
        printf("%5.2f ", entrada[i]);
    }

    printf("\n\nh[k] (kernel, normalizado /%d):\n  ", GANANCIA_KERNEL);
    for (i = 0; i < KERNEL_TAM; i++) {
        printf("%5.2f ", kernel[i]);
    }

    printf("\n\ny[n] (salida cruda):\n  ");
    for (i = 0; i < MUESTRAS_SALIDA; i++) {
        printf("%5.2f ", salida[i]);
    }

    printf("\n\nyN[n] (salida normalizada):\n  ");
    for (i = 0; i < MUESTRAS_SALIDA; i++) {
        printf("%5.2f ", (float)salida[i] / GANANCIA_KERNEL);
    }
